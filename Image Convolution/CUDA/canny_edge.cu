...
/**
  Use to calculate gaussian kernel. It only need to compute once.
  For parallel executing, make this function separate from gaussian_blur().

  @param[out] kernel      kernel matric for Gaussian blur use.
  
**/
void get_gaussian_kernel(float  *kernel)
{
  int    center, idx;
  float  sum;
  float  x, y, fx;
...
/**
  ...
  For parallel, each core handle one pixel.
  ...
  @param[in] kernel       kernel matric for Gaussian blur use.

**/
__global__ void cuda_gassian_blur(unsigned char *image, unsigned char *result_img, float *kernel)
{
  ...
  x = thread_id % IMAGE_WIDTH;
  y = thread_id / IMAGE_WIDTH;
  center = (K_SIZE) / 2;
  ...
  for (int gx = 0; gx < K_SIZE; gx++) {
    for (int gy = 0; gy < K_SIZE; gy++) {
      ...
      if (!(OUT_OF_BOUNDARY(px, py))) {
        sum += ((float)(image[px + py * IMAGE_WIDTH])) * kernel[gx + gy * K_SIZE];
      }
    }
  }

  result_img[thread_id] = (unsigned char)(sum + 0.5);  // Round up
}
...
**/
__global__ void cuda_sobel_filter(unsigned char *image, unsigned char *gradient, unsigned char *gradient_angle)
{
  int            x, y;
  int            thread_id = blockIdx.x * blockDim.x + threadIdx.x;
  unsigned int   theta;
  ...
  for (int dx = 0; dx < CONV_LEN; dx++) {
    for (int dy = 0; dy < CONV_LEN; dy++) {
      conv_idx = (thread_id - CONV_LEN / 2 + dx) + (CONV_LEN / 2 + dy) * IMAGE_WIDTH;
  ...
  gradient[thread_id] = (unsigned int)(sqrt(gx * gx + gy * gy) + 0.5);  // Round up

  theta = (atan(gy / gx) * 180 / PI + 0.5);  // Round up
  theta %= 180;

  if (theta <= 22.5 || theta > 157.5) {
    gradient_angle[thread_id] = (unsigned int)DEG_0;
  } else if (theta <= 67.5) {
    gradient_angle[thread_id] = (unsigned int)DEG_45;
  } else if (theta <= 112.5) {
    gradient_angle[thread_id] = (unsigned int)DEG_90;
  } else if (theta <= 157.5) {
    gradient_angle[thread_id] = (unsigned int)DEG_135;
  } else {
    printf("*********************************\n");
...
**/
__global__ void cuda_non_max_suppress(unsigned char *gradient, unsigned char *gradient_angle, unsigned char *result_img)
{
  int           thread_id = blockIdx.x * blockDim.x + threadIdx.x;
  int           x, y;
  unsigned int  cmp_idx_1, cmp_idx_2;
  ...
  switch (gradient_angle[thread_id]) {

  // gradient magnitude must greater than "east" and "west"
  case (DEG_0):
    cmp_idx_1 = thread_id + 1;
    cmp_idx_2 = thread_id - 1;
    break;

  // gradient magnitude must greater than "north east" and "south west"
  case (DEG_45):
    cmp_idx_1 = thread_id + IMAGE_WIDTH + 1;
    cmp_idx_2 = thread_id + IMAGE_WIDTH - 1;
    break;

  // gradient magnitude must greater than "north" and "south"
  case (DEG_90):
    cmp_idx_1 = thread_id + IMAGE_WIDTH;
    cmp_idx_2 = thread_id - IMAGE_WIDTH;
    break;
  ...
  if (gradient[thread_id] > gradient[cmp_idx_1] && gradient[thread_id] > gradient[cmp_idx_2]) {
    result_img[thread_id] = 0;
  } else {
    result_img[thread_id] = gradient[thread_id];
  }
}
...
**/
__global__ void cuda_double_threshold(unsigned char *image, unsigned char *result_img)
{
  int  thread_id = blockIdx.x * blockDim.x + threadIdx.x;
  int  x, y;
  ...
  if (image[thread_id] > HIGH_TH) {
    result_img[thread_id] = STRONG_EDGE_PIXEL;
  } else if (image[thread_id] > LOW_TH) {  // low < magnitude < high 
    result_img[thread_id] = WEAK_EDGE_PIXEL;
  } else {  // low > magnitude
    result_img[thread_id] = 0;
  }
}
...
**/
__global__ void cuda_hysteresis_track(unsigned char *image, unsigned char *result_img)
{
  int  thread_id = blockIdx.x * blockDim.x + threadIdx.x;
  int  x, y;
  ...
  // blob analysis
  if (image[thread_id] == WEAK_EDGE_PIXEL) {
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        if (image[thread_id + j * IMAGE_WIDTH + i] == STRONG_EDGE_PIXEL) {
          result_img[thread_id] = STRONG_EDGE_PIXEL;
          continue;
        }
      }
  ...
  } else if (image[thread_id] == STRONG_EDGE_PIXEL) {
    result_img[thread_id] = STRONG_EDGE_PIXEL;
  } else {  // Not edge
    result_img[thread_id] = 0;
  }
...
/**
  ...
  For parallel, this function also response for CUDA mem allocate
  and transfer data from/to host mem.
...
{
  float            cost_time[TIME_MAX], *kernel, *d_kernel;
  struct timespec  time;
  unsigned char    *d_img, *d_tmp, *d_tmp_2;
  int              img_sz, k_matrix_sz, threads, grid;
#if OUTPUT_EACH_IMG
  FILE  *fp_dbg = NULL;
#endif
  ...
  // CUDA Init
  //
  img_sz = IMAGE_WIDTH * IMAGE_HEIGHT * sizeof(char);
  threads = 32;
  grid = (img_sz + threads - 1) / threads;
  k_matrix_sz = K_SIZE * K_SIZE * sizeof(float);

  cudaMalloc((void **)&d_img, img_sz);
  cudaMalloc((void **)&d_tmp, img_sz);
  cudaMalloc((void **)&d_tmp_2, img_sz);
  cudaMalloc((void **)&d_kernel, k_matrix_sz);

  kernel = (float *)malloc(k_matrix_sz);

  // Every time get new image, need copy to CUDA mem.
  // Use to estimate a part of runtime latency
  start_time(&time);
  cudaMemcpy(d_img, image, img_sz, cudaMemcpyHostToDevice);
  cudaMemcpy(d_tmp, temp, img_sz, cudaMemcpyHostToDevice);
  cudaMemcpy(d_tmp_2, temp_2, img_sz, cudaMemcpyHostToDevice);

  cudaDeviceSynchronize();
  cost_time[TIME_CUDA_INIT] = end_time_ms(time);
  ...
  get_gaussian_kernel(kernel);
  cudaMemcpy(d_kernel, kernel, k_matrix_sz, cudaMemcpyHostToDevice);

  cuda_gassian_blur <<< grid, threads >>>(d_img, d_tmp, d_kernel);

  cudaDeviceSynchronize();
  cost_time[TIME_GAUSSIAN_BLUR] = end_time_ms(time);
  
#if OUTPUT_EACH_IMG
  cudaMemcpy(temp, d_tmp, img_sz, cudaMemcpyDeviceToHost);

  fp_dbg = fopen("lena512_gaussian_cuda.raw", "wb");
  ...
  //
  start_time(&time);
  cuda_sobel_filter <<< grid, threads >>>(d_tmp, d_img, d_tmp_2);
  cudaDeviceSynchronize();
  cost_time[TIME_SOBEL_FILTER] = end_time_ms(time);

#if OUTPUT_EACH_IMG
  cudaMemcpy(image, d_img, img_sz, cudaMemcpyDeviceToHost);
  cudaMemcpy(temp_2, d_tmp_2, img_sz, cudaMemcpyDeviceToHost);

  fp_dbg = fopen("lena512_sobel_gradient_cuda.raw", "wb");
  fwrite(image, 1, IMAGE_SIZE, fp_dbg);

  fp_dbg = fopen("lena512_sobel_angle_cuda.raw", "wb");
  ...
  //
  start_time(&time);
  cuda_non_max_suppress <<< grid, threads >>>(d_img, d_tmp_2, d_tmp);
  cudaDeviceSynchronize();
  ...
  fp_dbg = fopen("lena512_non_max_cuda.raw", "wb");
  fwrite(temp, 1, IMAGE_SIZE, fp_dbg);
#endif
  ...
  //
  start_time(&time);
  cuda_double_threshold <<< grid, threads >>>(d_tmp, d_tmp_2);
  cudaDeviceSynchronize();
  ...
  fp_dbg = fopen("lena512_double_th_cuda.raw", "wb");
  fwrite(temp_2, 1, IMAGE_SIZE, fp_dbg);
#endif
  ...
  //
  start_time(&time);
  cuda_hysteresis_track <<< grid, threads >>>(d_tmp_2, d_img);  // result return into image
  cudaDeviceSynchronize();
  ...
  fp_dbg = fopen("lena512_hys_track_cuda.raw", "wb");
  fwrite(image, 1, IMAGE_SIZE, fp_dbg);
...
int main()
{
  ...
  image_data = (unsigned char*)malloc(sizeof(char) * frame_size);
  ...
  fp_result = fopen("lena512_edge_cuda.raw", "wb");
